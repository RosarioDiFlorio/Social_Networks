...
def test_result_matching():
    ...
    with open(f) as file:
        while True:
            ...
            result = best_match(query, 0.02, inverted_db)
            result_opt = best_match_opt_k(query, 0.02, inverted_db,total_len_docs,k)
            #result_opt = best_match_opt(query, 0.02, inverted_db,total_len_docs)
...
def test_time_matching_k():
    ...
    for f in files:
        with open(f) as file:
            ...
            #print ("Num Query -> " +str(nquery))
            #print ("Time best_match -> "+ str(float(elapsed_global/nquery)))
            print (str(float(elapsed_opt_global/nquery)))
            #print("--------------------------")    

test_time_matching_k()
#test_time_matching()
